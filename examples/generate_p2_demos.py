...
from core.engine import (
    ...
    ellipse, save_excalidraw, save_obsidian_md, below, bounds,
)
...
def demo_svg_converter():
    ...
    shapes_y = 120
    shapes_h_rect = 80   # viewBox 100x80
    shapes_h_square = 100 # viewBox 100x100

    # Rectangle from SVG
    svg_rect = '<svg viewBox="0 0 100 80"><rect x="10" y="10" width="80" height="60" fill="#a5d8ff" stroke="#1971c2"/></svg>'
    rect_els = svg_to_elements(svg_rect, x=30, y=shapes_y, scale=1.0, stroke="#1971c2")
    rect_label = text_standalone(70, below(shapes_y, shapes_h_rect), "SVG <rect>", fs=12, color="#868e96")

    # Circle from SVG
    svg_circle = '<svg viewBox="0 0 100 100"><circle cx="50" cy="50" r="40" fill="#b2f2bb" stroke="#2f9e44"/></svg>'
    circle_els = svg_to_elements(svg_circle, x=180, y=shapes_y, scale=1.0, stroke="#2f9e44")
    circle_label = text_standalone(220, below(shapes_y, shapes_h_square), "SVG <circle>", fs=12, color="#868e96")

    # Triangle from SVG path
    svg_tri = '<svg viewBox="0 0 100 100"><path d="M50 10 L90 90 L10 90 Z" fill="#ffd8a8" stroke="#e8590c"/></svg>'
    tri_els = svg_to_elements(svg_tri, x=320, y=shapes_y, scale=1.0, stroke="#e8590c")
    tri_label = text_standalone(360, below(shapes_y, shapes_h_square), "SVG <path>", fs=12, color="#868e96")

    # --- Section 2: Bezier curves ---
    section2_y = below(shapes_y, shapes_h_square, gap=40)
    section2 = text_standalone(200, section2_y, "Bezier curves → polyline elements", fs=14, color="#868e96")

    curves_y = below(section2_y, 14, gap=10)
    curves_h = 100  # viewBox 200x100
    ...
    curve_els = svg_to_elements(svg_curve, x=50, y=curves_y, scale=1.0, stroke="#7048e8")
    curve_label = text_standalone(150, below(curves_y, curves_h), "Cubic Bezier curve", fs=12, color="#868e96")

    # --- Section 3: Complex path ---
    section3 = text_standalone(430, section2_y, "Star from SVG path", fs=14, color="#868e96")
    star_h = 80  # viewBox 100x100 at scale 0.8
    ...
    star_els = svg_to_elements(svg_star, x=440, y=curves_y, scale=0.8, stroke="#f08c00")
    star_label = text_standalone(470, below(curves_y, star_h), "Complex polygon", fs=12, color="#868e96")
...
def demo_icon_library():
    ...
    f1_mw = 420  # frame width 460 - 2*20 padding
    steps = [
    ...
    pipeline_y = 340
    pipeline_h = 50
    step_a = labeled_rect(30, pipeline_y, 130, pipeline_h, "SVG File", fill="#a5d8ff", stroke="#1971c2")
    step_b = labeled_rect(200, pipeline_y, 130, pipeline_h, "Convert", fill="#b2f2bb", stroke="#2f9e44")
    step_c = labeled_rect(370, pipeline_y, 130, pipeline_h, "Save to\nLibrary", fill="#ffd8a8", stroke="#e8590c")
    step_d = labeled_rect(540, pipeline_y, 130, pipeline_h, "Search", fill="#fcc2d7", stroke="#c2255c")
    step_e = labeled_rect(710, pipeline_y, 130, pipeline_h, "Load &\nPlace", fill="#d0bfff", stroke="#7048e8")
    ...
    # Code snippet labels — use below() to avoid overlap with pipeline boxes
